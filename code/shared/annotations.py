...
Adapted from zetta_utils.layer.volumetric.annotation.annotations
"""
...
class ShardingSpec:
    ...
    @staticmethod
    def murmur_hash(v, preshift_bits):
        v_shifted = v >> preshift_bits
        # v_numbits = v_shifted.bit_length()
        v_length = 8  # math.ceil(v_numbits / 8)
        v_bytes = v_shifted.to_bytes(v_length, 'little')
        hashed_v = mmh3.hash128(v_bytes, x64arch=False)
        hashed_v = hashed_v & ((1 << 64) - 1)  # Only keep the lower 64-bits
        return hashed_v

    def get_shard_number(self, chunk_id: int, include_minishard_number=False) -> int:
        """
        ...
        if self.hash == "identity":
            ...
            hashed_chunk_id = ShardingSpec.murmur_hash(chunk_id, self.preshift_bits)
...
class Annotation(ABC):
    ...
    def in_bounds(
        ...
        :param bounds: VolumetricIndex or geometry.BBox3D
    ...
    def convert_coordinates(self, from_res, to_res) -> None:
        ...
        :param from_res: Source resolution (geometry.Vec3D)
        :param to_res: Target resolution (geometry.Vec3D)
        """
    ...
    def read(
        ...
        if type == "POINT":
            ...
            result = PolyLineAnnotation.read_geometry(in_stream)
        else:
            raise ValueError(f"type: expected POINT, LINE, or POLYLINE, but got '{type}'")
        if property_specs:
...
class PointAnnotation(Annotation):
    ...
    def in_bounds(
        ...
        :param bounds: geometry.BBox3D or VolumetricIndex.
        ...
        if hasattr(bounds, "resolution"):
            ...
            from_res = geometry.Vec3D(*resolution)
            position = tuple(round(geometry.Vec3D(*self.position) * from_res / to_res, VEC3D_PRECISION))
    ...
    def convert_coordinates(self, from_res, to_res) -> None:
        ...
        :param to_res: Target resolution (geometry.Vec3D)
        """
        self.position = tuple(round(geometry.Vec3D(*self.position) * from_res / to_res, VEC3D_PRECISION))
    ...
    def with_converted_coordinates(self, from_res, to_res) -> "PointAnnotation":
        ...
        :param to_res: Target resolution (geometry.Vec3D)
        """
        new_position = tuple(round(geometry.Vec3D(*self.position) * from_res / to_res, VEC3D_PRECISION))
...
class LineAnnotation(Annotation):
    ...
    def in_bounds(
        ...
        if hasattr(bounds, "resolution"):
            to_res = bounds.resolution
            from_res = geometry.Vec3D(*resolution)
            start = tuple(round(geometry.Vec3D(*self.start) * from_res / to_res, VEC3D_PRECISION))
            end = tuple(round(geometry.Vec3D(*self.end) * from_res / to_res, VEC3D_PRECISION))
        else:
        ...
        if strict:
            ...
            if isinstance(bounds, geometry.BBox3D):
    ...
    def convert_coordinates(self, from_res, to_res) -> None:
        ...
        :param to_res: Target resolution (geometry.Vec3D)
        """
        self.start = tuple(round(geometry.Vec3D(*self.start) * from_res / to_res, VEC3D_PRECISION))
        self.end = tuple(round(geometry.Vec3D(*self.end) * from_res / to_res, VEC3D_PRECISION))
    ...
    def with_converted_coordinates(self, from_res, to_res) -> "LineAnnotation":
        ...
        :param to_res: Target resolution (geometry.Vec3D)
        """
        new_start = tuple(round(geometry.Vec3D(*self.start) * from_res / to_res, VEC3D_PRECISION))
        new_end = tuple(round(geometry.Vec3D(*self.end) * from_res / to_res, VEC3D_PRECISION))
...
class PolyLineAnnotation(Annotation):
    """PolyLine annotation represented by multiple point positions."""

    num_points: int = 0
    points: Sequence[Sequence[float]] = ()

    GEOMETRY_BYTES: int = 0  # 3 floats per point

    # We define an explicit initializer in order to control the parameter order:
    # num_points and points are required; id, properties, and relations are optional.
    def __init__(
        self,
        num_points: int = 0,
        points: Sequence[Sequence[float]] = (),
        ...
        self.points = points
        self.GEOMETRY_BYTES = 4 + self.num_points * 12  # 4 for num_points and 3 4s for each point

    def write_geometry(self, output: IO[bytes]) -> None:
        """Write num_points as uint32le and then each point as float32le values."""
        output.write(struct.pack("<I", int(self.num_points)))
        for point in self.points:
            output.write(struct.pack("<3f", *point))
    ...
    @classmethod
    def read_geometry(cls, in_stream: IO[bytes]) -> "PolyLineAnnotation":
        """Read line geometry from binary format."""
        num_points = struct.unpack("<I", in_stream.read(4))
        points = []
        for i in range(num_points):
            point = struct.unpack("<3f", in_stream.read(12))
            points.append(point)
        return cls(num_points=num_points, points=points)

    def in_bounds(
        self, bounds, resolution: Sequence[float] | None = None, strict: bool = False
    ) -> bool:
        """Return whether this polyline anywhere intersects the given bounds.

        :param bounds: VolumetricIndex or BBox3D
        :param line_resolution: Resolution of the points of this line.
        """
        if resolution is None:
            resolution = [1, 1, 1]
        points = []
        if hasattr(bounds, "resolution"):
            ...
            for point in self.points:
                points.append(tuple(round(Vec3D(*point) * from_res / to_res, VEC3D_PRECISION)))
        else:
            for point in self.points:
                points.append(tuple(point))
        if strict:
            return sum([1 if bounds.contains(point) else 0 for point in points]) == self.num_points
        else:
            if isinstance(bounds, BBox3D):
                for point_i in range(self.num_points - 1):
                    point_a = points[point_i]
                    point_b = points[point_i + 1]
                    if bounds.line_intersects(point_a, point_b, resolution=(1, 1, 1)):
                        return True
                return False
            else:
                for point_i in range(self.num_points - 1):
                    point_a = points[point_i]
                    point_b = points[point_i + 1]
                    if bounds.line_intersects(point_a, point_b):
                        return True
                return False
    ...
    def convert_coordinates(self, from_res, to_res) -> None:
        ...
        for point_i in range(self.num_points):
            self.points[point_i] = tuple(round(Vec3D(*self.points[point_i]) * from_res / to_res, VEC3D_PRECISION))

    def with_converted_coordinates(self, from_res, to_res) -> "PolyLineAnnotation":
        """Return new LineAnnotation with converted coordinates (immutable).
        ...
        new_points = []
        for point in self.points:
            new_points.append(tuple(round(Vec3D(*point) * from_res / to_res, VEC3D_PRECISION)))
        return PloyLineAnnotation(
            id=self.id,
