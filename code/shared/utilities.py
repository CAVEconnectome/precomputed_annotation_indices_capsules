"""
Helper utilities for the precomp_anno library.
Adapted from zetta_utils.layer.volumetric.annotation.utilities
"""
...
def write_bytes(file_or_gs_path: str, data: bytes):
    ...
    if not CLOUDFILES_AVAILABLE:
        raise RuntimeError("utitilies.write_bytes() depends on the cloudfiles module, which is not available. Please install it.")
...
def compressed_morton_code(cell_index, grid_shape):
    ...
    if isinstance(cell_index, str):
        raise ValueError("cell_index is a str, probably of the form 0_0_0, not a list")
